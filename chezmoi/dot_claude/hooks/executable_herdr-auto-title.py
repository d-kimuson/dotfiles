...
Claude Code / Codex / pi の UserPromptSubmit 相当のフックとして動作し、会話内容
から短いタイトルを生成して herdr のタブ名 (tab.rename) に反映する。

hook は即座に制御を返し、タイトル生成はバックグラウンドで走る。

Vendored from https://github.com/sh1ma/herdr-auto-title (MIT License,
Copyright (c) 2026 sh1ma) with local modifications:
  - pi agent support: pi セッション (JSONL) からのプロンプト抽出と
    `pi -p` によるタイトル生成バックエンドを追加。
  - 子プロセス側での再入防止として HERDR_AUTO_TITLE_DISABLE を付与。
"""
...
def _text_chunks(content) -> list[str] | None:
    """LLM 向け content (文字列または text パートの配列) から本文だけを取り出す。"""
...
def claude_user_prompts(transcript_path: str | None) -> list[str]:
    ...
    for entry in iter_transcript_entries(transcript_path):
        ...
        chunks = _text_chunks(entry.get("message", {}).get("content"))
        if chunks is None:
            continue
...
def pi_user_prompts(transcript_path: str | None) -> list[str]:
    """pi のセッション (JSON Lines) から人間の入力だけを時系列で取り出す。

    セッションは {"type": "message", "message": {"role": ..., "content": ...}}
    の行が時系列に並ぶ。Claude Code と同じ content 形式なので本文の取り出し方は
    共通化している。
    """
    prompts: list[str] = []
    for entry in iter_transcript_entries(transcript_path):
        ...
        if not isinstance(message, dict) or message.get("role") != "user":
            continue
        chunks = _text_chunks(message.get("content"))
        if chunks is None:
            continue
        cleaned = clean_prompt_text(" ".join(chunks))
        if cleaned:
...
def extract_user_prompts(transcript_path: str | None, agent: str) -> list[str]:
    ...
    if agent == PI:
        return pi_user_prompts(transcript_path)
...
AGENT_DISPLAY_NAMES = {CODEX: "Codex", PI: "Pi"}


def user_message_for(conversation_log: str, agent: str) -> str:
    name = AGENT_DISPLAY_NAMES.get(agent, "Claude Code")
    if LANG == JA:
        return (
...
def child_env() -> dict:
    ...
    # 生成呼び出しがこの hook や pi extension を再入させないための保険
    env["HERDR_AUTO_TITLE_DISABLE"] = "1"
    return env
...
def generate_with_pi(conversation_log: str, model: str) -> str:
    ...
    extensions / skills / MCP / コンテキストファイルを読み込ませないので、この
    呼び出しが herdr-auto-title extension を再入させることはない。子プロセス側の
    環境から HERDR_* も外しているため、herdr の agent 状態連携も動かない。
    """
    command = [
        "pi",
        "-p",
    ...
    if model:
        command += ["--model", model]
    command.append(user_message_for(conversation_log, PI))
    completed = run_generator(command, None)
    if completed is None:
        return ""
    if completed.returncode != 0:
...
def resolve_backend(agent: str) -> str:
    """どの CLI でタイトルを作るか決める。"""
    if BACKEND in (PI, CLAUDE, CODEX):
        return BACKEND
    # 既定は hook を呼んだエージェント自身。無ければ他のエージェントに回す
    others = [candidate for candidate in (PI, CLAUDE, CODEX) if candidate != agent]
    for candidate in (agent, *others):
        if which(candidate):
            return candidate
    return agent
...
def detect_agent(hook_input: dict) -> str:
    ...
    pi extension は明示的に agent: "pi" を付けて呼ぶ。
    """
