...
description: 日本語の技術文書・書籍原稿・技術記事の執筆規範。書籍の章や解説文向けの文章規範（整形、パラグラフライティング、論証の厳密さ、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除）と、箇条書きコンテンツを自然な文体の技術記事（ブログ記事）に仕上げるための執筆ガイドラインの両方を定める。日本語で技術書の章、草稿、記事、解説文を書く・推敲・リライトするとき、または箇条書きから技術記事を書き起こすときに使用する。
...
このスキルは二つのモードをカバーする。成果物の性質に応じて使い分ける。

- **文章規範モード**: 書籍の章、技術解説文、しっかりした地の文の記事を書く・推敲するときに従う規範。
- **記事執筆モード**: 箇条書きの技術コンテンツを、自然な文体のカジュアルな技術記事（ブログ記事）に仕上げるためのガイドライン。

両モードは文体の要求が異なる（文章規範モードは硬い地の文と論証の厳密さを求め、記事執筆モードはです・ます調のカジュアルな個人的トーンを求める）。混同せず、書く対象に合わせて適用するモードを選ぶ。
...
箇条書きの技術コンテンツを、AI生成っぽさを避けた自然で読みやすい技術記事（ブログ記事）に仕上げるときは、以下のガイドラインに従う。技術的な正確さを保ちながら、人が書いたような文章を目指す。
...
**CRITICAL**: 記事を書く前に、以下の4つの参考記事をすべて読むこと。

文体は言語化では完全に捉えられない。必ず4つすべての実際の記事を読んで、その流れ、表現、リズムを学習する。

Use the Skill tool to view ALL reference articles:
```
view(skill='skills/japanese-tech-writing/examples/sample-article-1.md')
view(skill='skills/japanese-tech-writing/examples/sample-article-2.md')
view(skill='skills/japanese-tech-writing/examples/sample-article-3.md')
view(skill='skills/japanese-tech-writing/examples/sample-article-4.md')
```

参考記事を読むときは、次の点に注目する:
- 自然な段落遷移（「さて、」「で、」「そんな中、」「という感じで、」「といったことがありました」）
- 個人的な声とカジュアルな表現（「〜してみました」「〜だったりします」「〜ですね」「〜が便利です」）
- 著者がリストと文章のどちらを使い分けているか
- コードブロックの自然な導入と説明の仕方
- 導入から結論までの会話的な流れ
- 個人的な逸話や動機（「退屈な開発をサボりたくて」「かくいう私も」）
